...
When the owner asks for a ground-up rewrite, the topic and structure are what change. His scenes, jokes, facts, and photos in their existing order all stay unless he explicitly changes the material. The prose itself is composed fresh, in one hand: his scenes and dictated lines are material to write from, not sentences to preserve. Patching new lines into old prose accretes into 四不像 (owner, 2026-07-18); when he hands over a new topic or thread design for an existing post, the default is a fresh composition, not incremental edits. Save a labeled checkpoint before overwriting a full post. After writing, run the production preview, hand him its link, and name the structural choices he may want to reverse.
...
- Keep the primary file's `title`, `titleZh`, `titleEn`, `description`, `descriptionZh`, and `descriptionEn` aligned with the two bodies, and the sibling file's `title` aligned with its language title. The primary file is the `<slug>.md` with no language suffix (the sibling is `<slug>.en.md` or `<slug>.zh.md`); the site renders shared metadata from the primary file only, so a retitle in the sibling alone never shows.
...
Before handing over a preview link, proofread every changed sibling in full, not just the lines touched: zh punctuation, CJK-Latin spacing, casing, duplicated words, photo alt text made stale or wrong by the edit, and frontmatter sync. Mechanics only. Pre-existing empty or thin alt text on photos the edit did not touch is not mechanics: mention it once as a flagged item, and author new alt text (descriptive, in the file's language) only when the owner asks or when composing a post.
...
Nothing reaches `main`, and so the live site, from a writing or editing session without the preview gate and the owner's approval (see preview-posts, ship-posts, release-check). Working commits on a gate branch in its own worktree are fine; the gate sits between them and `main`. An initial request to publish is not approval. After the owner has received and reviewed the production-preview link, an explicit "approved", "push", "ship it", or "go" is the approval signal that starts the gate chain.

## Source

The AI-tell patterns are distilled from [op7418/humanizer-zh](https://github.com/op7418/humanizer-zh) (MIT, by 歸藏), itself based on Wikipedia's "Signs of AI writing" (WikiProject AI Cleanup). Upstream features that conflict with this blog's style (upstream's emoji ban, curly-quote ban, 50-point rubric, and bulk-rewrite workflow) were deliberately dropped.
